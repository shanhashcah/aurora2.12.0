...
const (
	hcnetCoreRunnerModeOnline hcnetCoreRunnerMode = iota
...
func newHcnetCoreRunner(config CaptiveCoreConfig, mode hcnetCoreRunnerMode) (*hcnetCoreRunner, error) {
	var fullStoragePath string
	if runtime.GOOS == "windows" || mode == hcnetCoreRunnerModeOffline {
...
func writeConf(captiveCoreToml *CaptiveCoreToml, mode hcnetCoreRunnerMode, location string) (string, error) {
...
func (r *hcnetCoreRunner) createCmd(params ...string) *exec.Cmd {
...
func (r *hcnetCoreRunner) closeLogLineWriters(cmd *exec.Cmd) {
